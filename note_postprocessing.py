...
def post_process(after_clean_path, pecha_name, text_type, post_process_path):
    text = Path(f"{after_clean_path}/{pecha_name}.txt").read_text(encoding='utf-8')
    new_text = post_process_text(text, text_type)
    Path(f"{post_process_path}/{pecha_name}").mkdir(parents=True, exist_ok=True)
    Path(f"{post_process_path}/{pecha_name}/{pecha_name}.txt").write_text(new_text, encoding='utf-8')